...
import { GET_DELETE_SERVICES } from 'constants/url';
...
const ContractTable: FC<Props> = props => {
  ...
  const checkAll = () => {
    ...
    if (count > rowsPerPage) {
      if (countChecked !== rowsPerPage) {
        ...
        contracts.map(contract => newBulkChecked.push(contract.id));
      } else {
  ...
  const deleteContract = async (selectedId: number) => {
    await actionWrapper(async () => {
      await axios.delete(GET_DELETE_SERVICES([selectedId]));
    }, 'delete contracts');
  };

  const bulkDeleteContract: React.MouseEventHandler<HTMLButtonElement> = async event => {
    await actionWrapper(async () => {
      await axios.delete(GET_DELETE_SERVICES(checked));
          ...
          <TableHead>
            <HeaderRow
              headers={[
                ...
                { label: 'Contract ID', pL: '10px', pR: '10px', verticalAlign: 'top' },
